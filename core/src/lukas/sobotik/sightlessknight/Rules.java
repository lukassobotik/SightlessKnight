...
public class Rules {
    ...
    static List<BoardLocation> getValidMoves(List<BoardLocation> legalMoves, BoardLocation selectedPieceLocation, Piece piece, Board board, boolean checkForChecks, boolean checkCastlingMoves) {
        ...
        switch (piece.type) {
            case PAWN:
                validMoves.addAll(Objects.requireNonNull(getValidPawnMoves(legalMoves, selectedPieceLocation, piece.team, board)));
                break;
            case BISHOP:
                validMoves.addAll(getValidBishopMoves(legalMoves, selectedPieceLocation, piece.team, board));
                break;
            case KNIGHT:
                validMoves.addAll(getValidKnightMoves(legalMoves, selectedPieceLocation, piece.team, board));
                break;
            case ROOK:
                validMoves.addAll(getValidRookMoves(legalMoves, selectedPieceLocation, piece.team, board));
                break;
            case KING:
                validMoves.addAll(getValidKingMoves(legalMoves, selectedPieceLocation, piece.team, board, checkCastlingMoves));
                break;
            case QUEEN:
                validMoves.addAll(getValidQueenMoves(legalMoves, selectedPieceLocation, piece.team, board));
                break;
        }
        if (checkForChecks) {
    ...
    public static List<BoardLocation> getValidPawnMoves(List<BoardLocation> legalPawnMoves, BoardLocation pawnLocation, Team team, Board board) {
        ...
        if (legalPawnMoves.size() > 0 && (forwardPoint.getY() == 7 || forwardPoint.getY() == 0)) {
            Array<BoardLocation> promotionMoves = new Array<>();
